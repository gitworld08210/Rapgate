...
/// Renders [child] only when the signed-in account holds server-side admin
/// access (an `admin_roles` row), otherwise renders [fallback] (nothing by
/// default).
///
/// This is a UI convenience, NOT a security control. Admin access is verified
/// again inside every privileged Edge Function, so a user who forces this
/// widget to render gains no actual capability — the admin screen would simply
/// show an empty queue and every action would fail with a 403.
class AdminGate extends StatefulWidget {
  const AdminGate({
    ...
    /// Attempt to claim the admin role on mount. The `claim-admin-role`
    /// function only grants it to a server-side allowlisted, email-verified
    /// account, so this is a no-op for ordinary users.
